...
7. **Ask VonBot — optional**: DM VonBot from his profile (no friendship needed, unlike a real person) and he replies with a real AI-generated response instead of staying silent. Uses Google Gemini's free tier (no credit card) via its REST API directly — get a free key from Google AI Studio (`aistudio.google.com`) and set `VONBOT_AI_API_KEY` on Render. Which model it uses is auto-detected (VonBot asks Gemini's own ListModels endpoint what your key has access to and picks a "flash" model from the result) rather than hardcoded, since Google's model names change over time and aren't the same for every account/region — set `VONBOT_AI_MODEL` only if you want to pin a specific one (see `backend/src/lib/vonbotAI.js`). Leaving `VONBOT_AI_API_KEY` unset just means DMing VonBot never gets a reply — nothing else depends on it. Two safety layers on top of whatever Gemini itself blocks: a message matching self-harm/suicide language never reaches the AI at all and gets a fixed, human-written response pointing to a trusted adult and the 988 crisis line instead (`SELF_HARM_RESPONSE` in that same file — edit it directly if you'd rather it name a specific person), and everything else that does reach Gemini goes through with its safety filters set to block sexual content, hate speech, harassment, and dangerous content at the strictest threshold.
...
- **@mentions**: typing `@username` in a post caption or a comment notifies that person and renders as a clickable link to their profile (see `frontend/src/lib/linkify.jsx`). A caption mention also adds the post to that person's profile under a **Tagged** tab, next to their own **Posts** tab — visible to anyone who could already see the post in their own feed (same friends/public rule as everywhere else), unless the tagged person turned off "Show posts I'm tagged in on my profile" in Settings, which only hides the tab from *other* people — they still get notified either way, and can always see it on their own profile themselves (see `show_tagged` on users, `post_mentions` in `database/schema.sql`)
...
- **VonBot**: an automated account that reposts one trending gaming/anime/movie/superhero-news image a day (pulled from a handful of public RSS feeds — IGN, MyAnimeList, SlashFilm, ScreenRant) as a public post, so the feed stays active on its own between real posts. Its very first post ever is a one-time intro explaining what it is and what it'll do, before it starts actually reposting on the next tick — see `backend/src/lib/vonbot.js` and the "VonBot" step under Deploying below for how it's triggered. It has a real bio and can have a real avatar/cover photo too — since it has no login of its own to edit them the normal way, a dev account gets photo-upload controls directly on its profile page (`/u/vonbot`, only visible once VonBot's account actually exists — its first successful tick creates it)
- **Clickable links in captions**: any `http(s)://` url sitting in a post's caption text renders as a real link, not plain text -- this is also what makes VonBot's reposted article links clickable, since a VonBot post is just a normal post whose caption happens to end with a url (see `frontend/src/lib/linkify.jsx`)
- **Ask VonBot**: DM VonBot like any friend (no friend request needed, see his profile) and he actually texts back, powered by a free AI API -- see the "Ask VonBot" step under Deploying above for the safety design and how to turn it on
...
- **Ask VonBot goes quiet, not broken, if the free AI tier's quota runs out** -- a failed or rate-limited call is caught and logged server-side, and VonBot just doesn't reply to that message rather than showing an error, same fail-open pattern as Cloudinary/link previews elsewhere in this app. It also only replies to true 1:1 DMs (not group chats) and waits a few seconds between replies in the same conversation so a burst of quick messages doesn't burn through the quota in one go.
